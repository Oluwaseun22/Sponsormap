...
// Search terms used to fetch the featured sponsors on initial load.
// Each term is designed to return the right company as the top Typesense hit.
const FEATURED_QUERIES = [
  "HSBC Holdings", "Google UK", "Amazon UK Services", "Microsoft Limited",
  "Deloitte LLP", "Monzo Bank", "Revolut Ltd", "Meta Platforms",
  "IBM United Kingdom", "Accenture UK",
];
...
const STORIES = [
  ...
  },
  {
    ...
    before: "The CSV has 141,000 rows. No sector column. No salary data. I spent a full weekend building a spreadsheet just to make it searchable. That shouldn't be necessary.",
...
function SponsorCard({ sponsor, index, isBookmarked, onBookmark }) {
  ...
  useEffect(() => {
    if (!open || !sponsor.jobCount || !isSignedIn || jobs !== null) return;
    setJobsLoading(true);
  ...
  }, [open, sponsor.jobCount, sponsor.slug, isSignedIn, jobs]);
    ...
    <div className="card-enter" style={{ animationDelay: `${Math.min(index * 0.045, 0.5)}s` }}>
      ...
      {open && (
        <div style={{ background: "var(--bg-card-hi)", border: "1px solid var(--accent-mid)", borderTop: "1px solid var(--border)", borderRadius: "0 0 var(--r-lg) var(--r-lg)", padding: "14px 16px", animation: "fadeIn 0.18s ease forwards" }}>
          {/* Live jobs section — auth gated */}
          {sponsor.jobCount > 0 && !isSignedIn && (
            <a
              href="/sign-in"
              style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 12px", borderRadius: "var(--r-sm)", background: "var(--bg-raised)", border: "1px solid var(--border)", color: "var(--t-muted)", fontSize: "13px", textDecoration: "none", marginBottom: "12px" }}
            >
              <span>🔒</span>
              <span>{sponsor.jobCount} live {sponsor.jobCount === 1 ? "job" : "jobs"} — Sign in to view</span>
            </a>
          )}
          {sponsor.jobCount > 0 && isSignedIn && (
            <div style={{ marginBottom: "12px" }}>
              ...
              {jobsLoading ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  ...
                  {sponsor.jobCount > 3 && (
                    <a
                      href={`/jobs?sponsorSlug=${encodeURIComponent(sponsor.slug)}`}
...
function SearchTool({ initialSearch, initialSector, showAI, setShowAI }) {
  ...
  // Fetch featured sponsors once on mount — 10 parallel searches, take top hit each
  useEffect(() => {
    Promise.all(
      FEATURED_QUERIES.map(q =>
        fetch(`/api/search?q=${encodeURIComponent(q)}&perPage=1`)
          .then(r => r.ok ? r.json() : null)
      ...
      const seen = new Set();
      setFeaturedSponsors(hits.filter(h => h && !seen.has(h.id) && seen.add(h.id)));
    ...
    <div style={{ maxWidth: "860px", margin: "0 auto", padding: "32px 20px 120px", position: "relative", zIndex: 1 }}>
      ...
      {view === "search" && <>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }} role="list" aria-label="Sponsor results">
          {loading || (isDefaultView && featuredLoading)
      ...
      <div style={{ maxWidth: "680px", margin: "0 auto", padding: "60px 20px 80px" }}>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--r-xl)", padding: "36px 32px", marginBottom: "20px", boxShadow: "var(--sh-card)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            {[
              { icon: "📄", point: "141,000-company CSV. No search. No filters. Just a raw file from the Home Office." },
